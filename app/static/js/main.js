...
function renderHome(content) {
    ...
    content.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;">
            <div><h1 class="page-title">Welcome to ${escapeHtml(DB.settings.lab_abbrev)}-IMS</h1><p class="page-subtitle">${escapeHtml(DB.settings.lab_name)} - Inventory Management System</p></div>
        ...
        <div class="home-cards">
            ${DB.settings.system_mode !== 'Single' ? `<div class="home-card departments" onclick="navigateTo('departmentStack')"><div class="icon"><svg width="32" height="32" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg></div><h2>Departments</h2><p>Browse inventory and activities by department</p></div>` : `<div class="home-card departments" onclick="navigateTo('facultyStack', {departmentId: '${singleDeptId}'})"><div class="icon"><svg width="32" height="32" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div><h2>Faculty & Studies</h2><p>Browse inventory and activities by PI</p></div>`}
    ...
    content.innerHTML = `
        ...
        <div class="card" style="max-width:600px;"><div class="card-body">
            <form onsubmit="saveSettings(event)">
                ...
                <div class="form-group"><label>Safe Backup Directory Path</label><input type="text" class="form-control" id="sysBackupPath" value="${escapeHtml(DB.settings.backup_path)}" placeholder="e.g., D:/Backups"></div>
...
window.saveSettings = async function(e) {
    e.preventDefault();
    const payload = { lab_name: document.getElementById('sysLabName').value, lab_abbrev: document.getElementById('sysLabAbbrev').value, backup_path: document.getElementById('sysBackupPath').value };
...
window.openProfileModal = function() {
    ...
    openModal('My Profile', `
        <form id="profileForm" onsubmit="saveProfile(event)">
            ...
            <div class="form-group"><label>Designation</label><select class="form-control" name="Designation">${DB.settings.currentRole==='Admin' ? '<option value="HoD">HoD</option>' : ''}<option value="Manager">Manager</option></select></div>
        </form>
